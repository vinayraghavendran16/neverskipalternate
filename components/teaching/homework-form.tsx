...
export function HomeworkForm({ subjects, classes, defaultDue }: { subjects: Subject[]; classes: ClassOption[]; defaultDue: string }) {
  ...
  return <form className="homework-form" action={action}>
    ...
    <fieldset className="class-picker"><legend>Assign to classes</legend>{matching.map((item) => <label key={`${subjectId}:${item.id}`}><input type="checkbox" name="class_ids" value={item.id} /><span>{item.label}</span></label>)}{!matching.length && <p>No classes use this subject yet.</p>}</fieldset>
