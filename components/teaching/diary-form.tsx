...
export function DiaryForm({ allocations, date, selectedAllocation, timetableEntryId = "", previous, current }: { allocations: Option[]; date: string; selectedAllocation?: string; timetableEntryId?: string; previous: Previous; current: Previous }) {
  const router = useRouter();
  const [state, action, pending] = useActionState(saveDiaryEntry, initial);
  const [topic, setTopic] = useState(current?.topic || ""); const [summary, setSummary] = useState(current?.summary || ""); const [objective, setObjective] = useState(current?.learningObjective || "");
