...
export function AttendanceSheet({ classId, date, students, sessionStatus }: { classId: string; date: string; students: Student[]; sessionStatus: string }) {
  ...
  return <form action={action} className="attendance-sheet">
    ...
    <div className="attendance-list">{students.map((student, index) => <div className={`attendance-row attendance-${statuses[student.id]}`} key={student.id}>
      ...
      <select aria-label={`Attendance for ${student.name}`} name={`status_${student.id}`} value={statuses[student.id]} onChange={(event) => setStatuses((current) => ({ ...current, [student.id]: event.target.value }))} disabled={locked}><option value="present">Present</option><option value="absent">Absent</option><option value="late">Late</option><option value="excused">Excused</option></select>
      <input aria-label={`Attendance note for ${student.name}`} maxLength={500} name={`reason_${student.id}`} defaultValue={student.reason} placeholder={statuses[student.id] === "present" ? "Optional note" : "Reason for exception"} disabled={locked} />
    </div>)}</div>
