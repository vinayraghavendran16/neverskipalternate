...
export function AnnouncementForm({ campuses }: { campuses: { id: string; name: string }[] }) {
  ...
  return <form className="workflow-form" action={action}>
    ...
    <fieldset className="class-picker"><legend>Audience</legend>{["teacher","parent","student","staff"].map(role => <label key={role}><input type="checkbox" name="audience" value={role} defaultChecked /><span>{audienceLabels[role]}</span></label>)}</fieldset>
