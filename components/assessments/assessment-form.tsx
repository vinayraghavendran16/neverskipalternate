...
export function AssessmentForm({ allocations, today, initialAllocationId }: { allocations: { id: string; label: string }[]; today: string; initialAllocationId?: string }) {
  ...
  return <form className="assessment-form" action={action}>
    <label className="field">Class and subject<select name="class_subject_id" defaultValue={initialAllocationId || allocations[0]?.id}>{allocations.map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select></label>
