...
export function MarksGrid({ assessmentId, maxMarks, initialRows, locked }: { assessmentId: string; maxMarks: number; initialRows: MarkRow[]; locked: boolean }) {
  ...
  return <form className="marks-workspace" action={action}>
    ...
    <div className="marks-toolbar"><div><strong>{rows.length} students</strong><span>{scored.length} scored · average {average.toFixed(1)} / {maxMarks}</span></div><span className="speed-note">Save your changes before leaving this page</span></div>
