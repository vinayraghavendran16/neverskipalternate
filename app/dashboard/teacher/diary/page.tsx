...
export default async function DiaryPage({ searchParams }: { searchParams: Promise<{ allocation?: string; period?: string; date?: string }> }) {
  ...
  const date = /^\d{4}-\d{2}-\d{2}$/.test(query.date || "") ? query.date! : schoolDate();
  const [staffResult, classResult, subjectResult, allocationsResult, diaryResult] = await Promise.all([
    ...
    supabase.from("lesson_diary_entries").select("id, class_subject_id, entry_date, topic, summary, learning_objective, status, updated_at").eq("organization_id", context.organizationId).order("entry_date", { ascending: false }).limit(40),
  ]);
