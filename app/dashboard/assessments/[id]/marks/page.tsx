...
export default async function MarksPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: schoolClass }, { data: subject }, { data: enrollments }, { data: marks }] = await Promise.all([
  ...
  const studentIds = (enrollments || []).map((item) => item.student_id); const { data: students } = studentIds.length ? await supabase.from("students").select("id, admission_number, first_name, last_name").in("id", studentIds).order("first_name") : { data: [] };
