...
const assessmentSchema = z.object({ class_subject_id: z.uuid(), title: z.string().trim().min(1).max(180), assessment_date: z.iso.date(), max_marks: z.coerce.number().positive().max(10000), weight_percent: z.coerce.number().min(0).max(100).optional() });
const rowSchema = z.array(z.object({ studentId: z.uuid(), status: z.enum(["scored", "absent", "not_applicable"]), marks: z.number().nullable(), note: z.string().max(500) })).max(1000);
...
export async function saveAssessmentMarks(_: AssessmentActionState, formData: FormData): Promise<AssessmentActionState> {
  ...
  const context = await getUserContext(); const supabase = await createClient();
  if (!context || !supabase || !z.uuid().safeParse(assessmentId).success) return { error: "Assessment not found." };
  ...
  if (assessment.status === "published") return { error: "Published marks are locked. An administrator must reopen the assessment." };
  ...
  if (!parsed.data.length || parsed.data.some((row) => !enrolled.has(row.studentId))) return { error: "The class roster changed. Refresh before saving marks." };
  ...
  const rows = parsed.data.map((row) => ({ organization_id: context.organizationId, assessment_id: assessmentId, student_id: row.studentId, marks: row.status === "scored" ? row.marks : null, result_status: row.status, note: row.note || null, marked_by: context.userId, marked_at: now }));
  const { error } = await supabase.from("assessment_marks").upsert(rows, { onConflict: "assessment_id,student_id" });
  if (error) return { error: error.message };
  const published = intent === "publish";
  const { error: assessmentError } = await supabase.from("assessments").update({ status: published ? "published" : "marks_open", published_at: published ? now : null }).eq("id", assessmentId);
