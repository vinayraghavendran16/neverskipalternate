...
export default async function AssessmentsPage() {
  ...
  const [staffResult, classResult, subjectResult, allocationsResult, assessmentsResult, enrollmentResult] = await Promise.all([
    ...
    supabase.from("class_enrollments").select("class_id", { count: "exact", head: true }).eq("organization_id", context.organizationId).eq("status", "active"),
  ]);
  if ([staffResult, classResult, subjectResult, allocationsResult, assessmentsResult, enrollmentResult].some((result) => result.error)) throw new Error("Assessments could not be loaded.");
