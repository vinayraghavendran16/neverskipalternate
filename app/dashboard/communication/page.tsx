...
export default async function CommunicationPage() {
  ...
  const deliveryManager = ["owner","administrator","principal"].includes(context.role);
  ...
  const deliveryResult=deliveryManager?await supabase.from("notification_deliveries").select("channel,status").eq("organization_id",context.organizationId).order("created_at",{ascending:false}).limit(1000):{data:[],error:null};
  if(deliveryResult.error)throw new Error("Delivery status could not be loaded.");
  const deliveries=deliveryResult.data||[],deliveryCount=(channel:string,status:string)=>deliveries.filter(item=>item.channel===channel&&item.status===status).length;
  ...
  return <AppShell context={context} activePath="/dashboard/communication" pageTitle="Communication">
    ...
    {deliveryManager&&<section className="student-metrics delivery-metrics"><div><span>In-app delivered</span><strong>{deliveryCount("in_app","delivered")}</strong><small>Recent recipient records</small></div><div><span>Email awaiting provider</span><strong>{deliveryCount("email","awaiting_provider")}</strong><small>No external sends</small></div><div><span>SMS awaiting provider</span><strong>{deliveryCount("sms","awaiting_provider")}</strong><small>No external sends</small></div><div><span>Failed deliveries</span><strong>{deliveries.filter(item=>item.status==="failed").length}</strong><small>Recent delivery records</small></div></section>}
