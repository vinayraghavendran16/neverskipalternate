...
async function SchoolReport({ context, query, period }: { context: UserContext; query: Search; period: string }) {
  ...
  const attentionStudents = studentMetrics.filter((row) => row.band.key === "review" || row.band.key === "watch");
...
function ClassTable({ rows }: { rows: { id: string; label: string; students: number; attendance: number | null; score: number | null; review: number; missing: number }[] }) {
  ...
  return <div className="report-table-wrap"><table className="report-table"><thead><tr><th>Class</th><th>Students</th><th>Attendance</th><th>Published avg.</th><th>Overdue work</th><th>Review</th><th><span className="sr-only">Open</span></th></tr></thead><tbody>{rows.map((row) => { const hasEvidence = row.attendance !== null || row.score !== null || row.missing > 0; return <tr key={row.id}><td><b>{row.label}</b></td><td>{row.students}</td><td>{fmt(row.attendance)}</td><td>{fmt(row.score)}</td><td>{row.missing}</td><td><span className={`signal-pill ${row.review ? "review" : hasEvidence ? "on-track" : "neutral"}`}>{row.review ? `${row.review} review` : hasEvidence ? "On track" : "No evidence"}</span></td><td><Link aria-label={`Open ${row.label}`} href={`/dashboard/academics/classes/${row.id}`}>→</Link></td></tr>; })}</tbody></table></div>;
}
