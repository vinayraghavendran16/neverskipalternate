...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ next?: string; error?: string; status?: string }> }) {
  ...
  const context = user ? await getUserContext() : null;
  if (user && isPlatformOwnerEmail(user.email) && (!context || !params.next || params.next === "/platform")) redirect("/platform");
  if (user && context) redirect(safeNextPath(params.next || "/dashboard"));
  ...
  return (
    <main className="login">
      ...
      <section className="login-panel">{params.error === "callback" && <p role="alert" className="form-error">This sign-in link expired or could not be verified. Request a new invitation from your administrator.</p>}{params.status === "inactive" && <p role="alert" className="form-error">Your account is complete, but school access is not active. Contact the school owner.</p>}<LoginForm configured={isSupabaseConfigured()} next={params.next || "/dashboard"} /></section>
    </main>
  );
}
